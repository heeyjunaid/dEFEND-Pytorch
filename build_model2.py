...
class dEFENDNet(nn.Module):
    def __init__(self, weight_matrix, max_sentence_length, max_comment_length, device, num_classes = 2, max_sentence_count = 50 ,max_comment_count = 150,batch_size = 32 ,embedding_dim = 100, latent_dim = 200):
        ...
        self.embedding = create_embeddeding_layer(weight_matrix)
    ...
    def forward(self, content, comment):
        ...
        print("sentence",self.sentence_encoder.weight_ih_l0)        
    ...
    def initHidden(self):
        ...
        word_lstm_weight = Variable(torch.zeros(2, self.batch_size, self.embedding_dim).to(self.device))
        comment_lstm_weight = Variable(torch.zeros(2, self.batch_size, self.embedding_dim).to(self.device))
